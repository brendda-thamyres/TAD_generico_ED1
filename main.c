    ...
    do {
        ...
        switch(opcao) {
            ...
            case 4: {
                ...
                Casa chave;
                printf("Coordenada para consulta: "); //pergunta a coordenada para a comparação
                while( scanf("%f %f", &chave.localizacao[0], &chave.localizacao[1]) != 2) {
                printf("\nFormato de coordenadas invalido! Tente um fomato do tipo - ex: 22.11 1212.442\n");
                while (getchar() != '\n');
               }
               getchar();

               //compara a chave com cada um dos itens na coleção
                int i;
                for(i = 0; i < col->quantAtual; i++) {
                    Casa *item = (Casa*)col->item[i];

                    if(comparaItem(item, &chave)) { // se encontra, mostra as informações
                        printf("\n--- Casa Encontrada ---");
                        imprimirCasa(item);
                        break;
            ...
            case 7: {
                ...
                } else if(col->quantAtual > 0) { //verifica se a coleção está vazia
